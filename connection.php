...
// Function to sanitize input to prevent XSS vulnerabilities
...
class Cart
{
    ...
    public function createInvoice($userId)
    {
        ...
        $pdf->Cell(60, 5, sanitizeInput($userDetails['firstname']) . " " . sanitizeInput($userDetails['lastname']), 0, 0, "L");
        $pdf->SetX(-60);
        $pdf->Cell(60, 5, "Date: " . date('Y-m-d'), 0, 1, "L");
        $pdf->SetFont('Courier', 'I', 12);
        $pdf->Cell(60, 5, sanitizeInput($userDetails['email']), 0, 1, "L");
        $pdf->Cell(60, 5, sanitizeInput($userDetails['phone']), 0, 1, "L");
        $pdf->SetFont('Courier', '', 14);
        $pdf->Ln(5);
        $pdf->Cell(60, 5, sanitizeInput($userAddress['line1']), 0, 1, "L");
        if ($userAddress['line2'] != '') {
            $pdf->Cell(60, 5, sanitizeInput($userAddress['line2']), 0, 1, "L");
        }
        $pdf->Cell(60, 5, sanitizeInput($userAddress['city']), 0, 1, "L");
        $pdf->Cell(60, 5, sanitizeInput($userAddress['province']), 0, 1, "L");
        $pdf->Cell(60, 5, sanitizeInput($userAddress['postal_code']), 0, 1, "L");
        ...
        foreach ($allCartItems as $singleItem) {
            ...
            $pdf->Ln(15);
            $pdf->SetX(10);
            $productName = sanitizeInput($item[0]['product_name']);
            $pdf->Cell(100, 15, $productName, 1, 0, "L");
        ...
        $pdf->SetFillColor(239, 240, 243);
        $pdf->Cell(30, 15, '$' . sanitizeInput($grandtotal), 1, 0, 'R', 1);

        $pdf->Ln(30);
        $pdf->SetX(-60);

        $file = 'invoices/' . sanitizeInput($userDetails['firstname']) . '_' . date('Y_m_d_H_i_s') . '.pdf';
